...
public class PathGroup extends Group {
	...
	private final SimpleBooleanProperty hasPointSelected = new SimpleBooleanProperty(
		false); // boolean property that turn true when a CirclePoint is supposedly selected.
	...
	public PathGroup(String pathName, Path path, Node drawingPane) {
		...
		pathTable.getSelectionModel().getSelectedItems()
			.addListener((ListChangeListener<Point>) event -> {
				if (event.next()) {
					if (event.wasAdded()) {
		...
		X_SCALE = DoubleExpression.doubleExpression(
			Bindings.divide(1, drawingPane.scaleXProperty())); // creates binding for x scale
		Y_SCALE = DoubleExpression.doubleExpression(
			Bindings.divide(1, drawingPane.scaleYProperty())); // creates binding for y scale
	}


	/**
	 * Adds points to the drawing pane and HashMap and binds the scale properties of the
	 * CirclePoint
	 *
