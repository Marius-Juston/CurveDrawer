...
import org.curvedrawer.util.RobotPath;
...
public class CurveDrawerTabController implements Initializable {
	...
	/**
	 * Sends the selected paths to Main.networktable NetworkTable instance as a converted string of
	 * poses.
	 */
	@FXML
	...
	/**
	 * Creates a path by asking for the path selector popup and adds it using the addPath method. If
	 * no path is selected for any reason it will return null.
	 *
	...
	/**
	 * Handles mouse clicked event. If the mouse is right clicking and is not dragging then if it is
	 * the first point ever it will ask to create a new path if not it will just add to the selected
	 * path the current mouse position as a path point
	 *
	...
	private void createPoint(MouseEvent mouseEvent) {
		if ((mouseEvent.getButton() == MouseButton.PRIMARY) && !isDragging.get()) {
			...
			Point point = new Point(
				drawingPane.parentToLocal(mouseEvent.getX(), mouseEvent.getY()));
	...
	/**
	 * Opens up the Path selector popup to ask for what type of path is wanted and what name it
	 * should by referred by.
	 *
	...
	/**
	 * If the pathName and the path are not null then it will create a pathgroup and add its
	 * titlepane to the path view and make that the expaneded node. It will also make the selected
	 * path to be the added path and finally it will add the PathGroup to the drawing pane.
	 *
		...
		if ((path != null) && (pathName != null)) {
			...
			path.getPoints().addListener((ListChangeListener<Point>) c -> {
				if (c.next()) {
					...
					pathPoints.put(path, path.createPathPoses());
					System.out
						.println(pathPoints.get(path).length + "\t" + path.getNumberOfSteps());
				}
			});
		}
	}
	...
	/**
	 * Scales the drawing pane given its direction and the default Main.ZOOM_FACTOR variable value.
	 * Negative direction means zoom out, positive direction means zoom in and 0 direction does not
	 * change the scaling.
	 *
	...
	/**
	 * Scales the drawing pane given its direction and amount. Negative direction means zoom out,
	 * positive direction means zoom in and 0 direction does not change the scaling.
	 *
	 * @param direction zoom in (+) or zoom out (-)
	...
	/**
	 * Removes the wanted Path by retrieving its associated PathGroup and removing the path group
	 * title pane from the path viewer and from the drawing pane
	 *
	...
	private void handleKeyPresses(
		KeyEvent event) { //TODO make it so that you do not need to manually focus the vBox or tab to be able to get input from user
		if (event.isControlDown()) {
			switch (event.getCode()) {
				...
				case DOWN:
					selectedPath.set(
						Math.min(selectedPath.getValue() + 1, pathsViewer.getPanes().size() - 1));
					break;
				case DIGIT0:
	...
	/**
	 * Pans the drawing pane given the current mouse location and the location that it was pressed
	 * first
	 *
	...
	private void pan(MouseEvent event) {
		if ((event.getButton() == MouseButton.PRIMARY) && !anyPointIsSelected()) {
			drawingPane
				.setTranslateX((drawingPane.getTranslateX() + event.getX()) - pressedX.get());
			drawingPane
				.setTranslateY((drawingPane.getTranslateY() + event.getY()) - pressedY.get());
	...
	public void showRobotPath(ActionEvent actionEvent) throws IOException {
		List<File> files = RobotPath
			.retrieveCSVFile(((Button) actionEvent.getSource()).getScene().getWindow());

		if (files != null) {
			for (File file : files) {
				CategoryDataset csv = new CSV()
					.readCategoryDataset(Files.newBufferedReader(file.toPath()));

				Pose[] targetPoses = new Pose[csv.getRowCount()];
				Pose[] realPoses = new Pose[csv.getRowCount()];
				System.out
					.println(csv.getRowIndex(csv.getRowCount() - 1, csv.getColumnCount() - 1));

				for (int i = 0; i < csv.getRowCount(); i++) {
					targetPoses[i] = new Pose(csv.getValue(i, 1), csv.getValue(i, 2));
					realPoses[i] = new Pose(csv.getValue(i, 4), csv.getValue(i, 5));
				}

				addPath("Robot Actual", new Path(csv.getRowCount()) {
					@Override
				...
				addPath("Robot Target", new Path(csv.getRowCount()) {
					@Override
					public Pose[] createPathPoses() {
						return targetPoses;
					}
