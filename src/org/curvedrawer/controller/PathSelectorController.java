...
public class PathSelectorController implements
	Initializable { //TODO make this better and more efficient
	...
	public static Entry<String, Path> getPathChoice(String[] existingPathNames) {
		...
		try {
			...
			String text = ((TextInputControl) ((Pane) parent.getChildrenUnmodifiable().get(0))
	...
	public final void initialize(URL location, ResourceBundle resources) {
		if (PathType.values().length > 0) {
			...
			pathTypeSelection.getItems().addAll(
				Arrays.stream(PathType.values()).map(PathType::name).toArray(String[]::new));
	...
	/**
	 * Converts the String selected path to a path type and creates an instance of the Path from
	 * that. closes the window after that
	 *
	...
	private void getPathAndClose(ActionEvent event) {
		try {
			selectedPath = (Path) PathType.valueOf(pathTypeSelection.getValue())
				.getAssociatedClass().getDeclaredConstructors()[0]
				.newInstance(Main.NUMBER_OF_STEPS, new Point[0]);
	...
	/**
	 * Method to turn the ok button to be disabled or enabled given the current text in the
	 * TextField and the path type selected.
	 *
	...
	/**
	 * Handles key event for the TextField. Checks if the new word has already been used already or
	 * not if it has already been used the TextField text will change to be red.
	 *
	...
	private void handleInput(KeyEvent event) {
		String text =
			pathName.getText()
				.concat((Character.isAlphabetic(event.getCharacter().charAt(0)) ? event
